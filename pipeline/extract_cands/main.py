...
def main():
	...
	ap.add_argument('--window-s', type=float, default=0.003,
					 help='fixed cutout window length, seconds (default 3 ms), '
	...
	ap.add_argument('--no-plot', dest='plot', action='store_false', default=True,
					 help='disable the polarimetric diagnostic PNG per candidate')
	ap.add_argument('-F', '--nchan', type=int, default=8,
					 help='number of channels for dspsr (-F) folding')
	...
	calib_file = None
	cal_db = None
	if not args.no_cal or args.calib or args.calib_db:
	...
	if bw_mhz > 0:
		...
		print("  WARNING: could not read BW from DADA header, "
			  "falling back to 4096 phase bins")
...
def _process_dspsr(c, event, i_event, n_events, frag,
	...
	dada_paths, seek_mjd, plan_note = plan_dspsr_fold(
		frags, c['mjd'], margin_s, period)
	...
	print(f"  mode                  : fast (cropped input, real period"
	...
	if len(dada_paths) == 1:
		...
		crop_dur_s = max(3.0 * (period + 2 * margin_s), 0.35) + _prepad_s
	...
	if calib_file or cal_db:
		...
		print("  (no calibration available: saving the UNCALIBRATED fast fold)")
...
def _calibrate_archive(ar_path, calib_file, cal_db, args, fold_bw_mhz):
	...
	fix_dspsr_receiver(ar_path)
	_exp_cbw = (fold_bw_mhz / args.nchan
				if fold_bw_mhz > 0 else 0.0)
	fscrunch_to_nchan(ar_path, args.nchan, expected_chan_bw=_exp_cbw)
